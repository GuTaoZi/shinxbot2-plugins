...
enum class DiceMode { SUM, MIN, MAX };
...
struct ExpTree {
    ...
    ExpTree(int v)
        : op(OpType::NUMBER), value(v), calcVal(v), left(nullptr),
          right(nullptr) {}
    ...
    bool reduce() {
        ...
        if (getPreority(op) > 1) {
            if (left &&
                (left->op != OpType::NUMBER &&
                     getPreority(left->op) < getPreority(op) ||
                 getPreority(left->op) == getPreority(OpType::D_BINARY))) {
                leftStr = "(" + leftStr + ")";
            }
            if (right &&
                (right->op != OpType::NUMBER &&
                     getPreority(right->op) < getPreority(op) ||
                 getPreority(right->op) == getPreority(OpType::D_BINARY))) {
                rightStr = "(" + rightStr + ")";
            }
        }
        renderedStr =
    ...
    bool doDice(DiceMode mode = DiceMode::SUM) {
        ...
        if (op == OpType::MINFUNC || op == OpType::MAXFUNC) {
            ...
            DiceMode childMode =
                op == OpType::MINFUNC ? DiceMode::MIN : DiceMode::MAX;
            if (right->op != OpType::NUMBER) {
                bool ret = right->doDice(childMode);
                if (right->op != OpType::NUMBER) {
                    renderedStr = fmt::format(
                        "{}({})", op == OpType::MINFUNC ? "min" : "max",
                        right->renderedStr);
                } else {
        ...
        if (op == OpType::MINLISTFUNC || op == OpType::MAXLISTFUNC) {
            ...
            renderedStr =
                fmt::format("{}(", op == OpType::MINLISTFUNC ? "min" : "max");
            for (size_t i = 0; i < args.size(); ++i) {
                if (i)
                    renderedStr += ",";
                renderedStr += args[i]->renderedStr;
            ...
            if (mode == DiceMode::SUM) {
                if (num >= 100) {
                    if (sides >= 1) {
                        ...
                        double stddev =
                            sqrt(num * (1ll * sides * sides - 1) / 12.0);
                        std::normal_distribution<double> dist(mean, stddev);
                        rval = std::round(dist(drng()));
                        if (rval < num)
                            rval = num;
                        if (rval > 1ll * num * sides)
                            rval = 1ll * num * sides;
                    }
                    ...
                    if (num <= 1) {
                        renderedStr =
                            fmt::format("({}d{}={})", num, sides, rval);
                    } else {
                        renderedStr = fmt::format("({}d{}={}=({}))", num, sides,
                                                  rval, renderedStr);
                    }
                }
            } else {
                ...
                if (num >= 100) {
                    if (sides >= 1) {
                        ...
                        if (mode == DiceMode::MAX) {
                            ...
                            rval = std::max(
                                1.0, std::floor((1.0 - pow(rrval, 1.0 / num)) *
                                                sides));
                        }
                    } else {
                        rval = 0;
                    ...
                    renderedStr = fmt::format(
                        "({}d{}{}={:.0f})", num, sides,
                        mode == DiceMode::MIN ? "min" : "max", rval);
                } else if (num >= 20) {
                    for (int i = 0; i < num; ++i) {
                        int rrval = dice_random(sides);
                    ...
                    if (num <= 1) {
                        ...
                        renderedStr = fmt::format(
                            "({}d{}{}={:.0f}=({}))", num, sides,
                            mode == DiceMode::MIN ? "min" : "max", rval, rolls);
                    }
                }
            }
            calcVal = rval;
...
class Parser {
    ...
    // primary     = number | '('expr')' | min'('expr_list')' |
    // max'('expr_list')' | min'('expr')' | max'('expr')' expr_list   = add_sub
    // (',' add_sub)*
    Lexer lexer;
    Token cur;
...
private:
    ...
    ExpTree *parsePrimary() {
        ...
        if (cur.type == TokenType::MIN_FUNC ||
            cur.type == TokenType::MAX_FUNC) {
            OpType func = cur.type == TokenType::MIN_FUNC ? OpType::MINFUNC
                                                          : OpType::MAXFUNC;
            next();
            if (cur.type != TokenType::LPAREN) {
                throw std::runtime_error("Missing (");
            ...
            while (cur.type == TokenType::COMMA) {
                if (func == OpType::MINFUNC)
                    func = OpType::MINLISTFUNC;
                else if (func == OpType::MAXFUNC)
                    func = OpType::MAXLISTFUNC;
                next();
